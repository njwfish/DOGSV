...
class VCFQuery:
    def __init__(self, query, dir, output, prefix_chrom, all_info):
        ...
        :param prefix_chrom: boolean, include chromosome prefix
        :param all_info: bool, include tool specific information
        """
        self.query = query
        self.maps = Maps(self.query.variants, self.query.variants_cursor, 0)
        self.maps.gen_dicts()
    ...
    def gen_cols(self):
        ...
        for t in self.tables:
            sql = "SELECT column_name, data_type, column_comment from information_schema.columns where " \
                  "table_schema='dogsv' and table_name='%s';" % t
            results = query_sql(sql, self.query.variants, self.query.variants_cursor)
            for c in results:
                if '_id' not in c:
    ...
    def get_by_id(self, id):
        ...
        record.update({'records': list(query_sql(sql, self.query.variants, self.query.variants_cursor)[0])})

        for t in self.tables:
            sql = "select * from %s where record_id = %d" % (t, id)
            if '_samples' in t or t == 'genotype':
                record.update({t.lower(): [[v for v in r]
                                           for r in query_sql(sql, self.query.variants, self.query.variants_cursor)]})
            else:
                record.update({t.lower(): list(query_sql(sql, self.query.variants, self.query.variants_cursor)[0])})
    ...
    def get_fks(self, ids):
        ...
        for ID in ids:
            ...
            for t in query_sql(sql, self.query.variants, self.query.variants_cursor):
                tool = self.maps.tool_mapping[t[0]]
                if t[0] not in TOOLS:
                    TOOLS[tool].append(ID)
                sql = "select sample_id from genotype where record_id = %s" % ID
                for s in query_sql(sql, self.query.variants, self.query.variants_cursor):
                    if s[0] not in SAMPLES[tool]:
                        SAMPLES[tool].append(s[0])
        return SAMPLES, TOOLS

    def sql_to_vcf(self):
        get_ids = "select id from %s" % self.query.tid
        results = [r[0] for r in query_sql(get_ids, self.query.variants, self.query.variants_cursor)]
        ...
        for tool in TOOLS.keys():
            ...
            with open(self.files[-1], 'w') as writer:
                ...
                writer.write('\n##INFO=<ID=CHR2,Number=1,Type=Integer,Description="Chromosome of the end position of '
                             'the variant described in this record.">')
                writer.write('\n##INFO=<ID=POS2,Number=1,Type=Integer,Description="Either the end of the variant or '
                             'the position of the translocation on the second chromosome.">')
                writer.write('\n##INFO=<ID=TYPE,Number=1,Type=String,Description="Type of structural variant.">')
                writer.write('\n##INFO=<ID=LEN,Number=1,Type=Integer,Description="The length of the structural '
                             'variant.">')
                for t in self.tables:
                    if '_info' in t:
                ...
                for v in query_sql(sql, self.query.variants, self.query.variants_cursor):
        ...
        with ZipFile(self.dir + self.output + '.zip', 'w') as out:
            for f in self.files:
                out.write(f)
                os.remove(f)
