...
class SQLtoVCF:
    def __init__(self, query, output, prefix_chrom, all_info):
        ...
        self.tables = [t.split(' ')[0] for t in query.lower().split('join ') if 'select' not in t]
    ...
    def sql_to_VCF(self):
        """Outputs the VCF header, including INFO, ALT and FORMAT.
            ...
            :type tsv: DictReader
        """
        getIDs = "select id from" + self.query.lower().split('from')[1]
        results = [r[0] for r in query_sql(getIDs, self.db, self.cursor)]
        SAMPLES = self.get_samples(results)
        with open(output, 'w+') as writer:
            ...
            vcf_fields = vcf_fields + [self.maps.sample_mapping[s] for s in SAMPLES]
