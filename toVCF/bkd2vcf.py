...
def gen_reader_dict():
    """Skip the header and generate a DictReader to parse teh tsv,
        save the commands for later use.
        :return DictReader: a list of dicts to facilitate easy data parsing
        :rtype: DictReader
    """
...
def tsv_to_vcf(tsv):
    """Outputs the VCF header, including INFO, ALT and FORMAT.
        Parses throught the Breakdancer input file, outputting in
        Variant Call Format.
        :param tsv: the dict reader created from a Breakdancer TSV
        :type tsv: DictReader
    """
    with open(files['ovcf'], 'w+') as writer:
        # Write the VCF header.
        writer.write('##fileformat=VCFv4.2')
        writer.write('\n##INFO=<ID=ORIENTATION1,Number=1,Type=String,Description='
                     '"The number of reads mapped to the plus (+) or the minus (-) strand in the anchoring regions.">')
        writer.write('\n##INFO=<ID=CHR2,Number=1,Type=Integer,Description='
                     '"Chromosome of the end position of the variant described in this record.">')
        writer.write('\n##INFO=<ID=POS2,Number=1,Type=Integer,Description='
                     '"End position of the variant described in this record.">')
        writer.write('\n##INFO=<ID=ORIENTATION2,Number=1,Type=String,Description='
                     '"The number of reads mapped to the plus (+) or the minus (-) strand in the anchoring regions.">')
        writer.write('\n##INFO=<ID=TYPE,Number=1,Type=String,Description='
                     '"Type of structural variant.">')
        writer.write('\n##INFO=<ID=SIZE,Number=.,Type=Integer,Description='
                     '"Size of the SV in base pairs.  It is meaningless for inter-chromosomal translocations.">')
        writer.write('\n##INFO=<ID=SCORE,Number=1,Type=Integer,Description='
                     '"The confidence score associated with the prediction.">')
        writer.write('\n##INFO=<ID=NUM_READS,Number=1,Type=Integer,Description='
                     '"Total number of supporting read pairs, across all samples.')
        ...
        writer.write('\n##FORMAT=<ID=NR,Number=1,Type=String,Description='
                     '"Number of supporting read pairs from this sample">')
        ...
        for record in tsv:
            ...
            POS = record['Pos1']
            ALT = '<%s>' % record['Type']
            INFO = 'ORIENTATION1=%s;CHR2=%s;POS2=%s;ORIENTATION2=%s;' \
                   'TYPE=%s;SIZE=%s;SCORE=%s;NUM_READS=%s;CMD=%s;METHOD=Breakdancer' \
                   % (record['Orientation1'], record['Chr2'], record['Pos2'], record['Orientation2'], record['Type'],
                      record['Size'], record['Score'], record['num_Reads'], command[0])
            FORMAT = 'NR:SC'
            SAMPLES = []
            ...
            records.append([CHROM, POS, '%s%08d' % (record['Type'], ID),
                            '.', ALT, '.', '.', INFO, FORMAT, "\t".join(SAMPLES)])
